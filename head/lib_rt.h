...
void					camera(register t_cam cam, t_ray *ray, register float x,
								register float y);
...
void					get_normal_object(t_obj *obj, register t_ray ray,
								register float det);
...
void					get_ort_obj_info(t_env *e, char **str, size_t *i,
								size_t type);
...
void					push_to_window(SDL_Renderer *rend, unsigned int *img,
								size_t w, size_t h);
cl_float3				reverse(register cl_float3 vect);
cl_float3				rotate_x(register cl_float3 point,
								register float angle);
cl_float3				rotate_y(register cl_float3 point,
								register float angle);
cl_float3				rotate_z(register cl_float3 point,
								register float angle);
...
char					*clustering_hash_contribution(size_t nbr_elem,
								size_t nbr_block);
size_t					cluster_get_contribution(size_t id, char *tab_work,
								t_img img, unsigned int *img_work, t_mimg mimg);
void					cluster_finish_contribution(size_t id, char *tab_work,
								t_img img, unsigned int *img_work, t_mimg mimg);
...
void					filtered_black_white(unsigned int *img,
								size_t resolution);
cl_float2				random2(cl_float2 c);
float					smooth_voronoi(cl_float2 x);
...
float					ellipsoid(register t_obj obj, register t_ray ray);
cl_float3				get_ellipsoid_normale(register t_obj *obj,
								register float det);
cl_float3				cross(register cl_float3 a, register cl_float3 b);
t_roots					solve_cubic_equation(register double a,
								register double b, register double c,
								register double d);
void					refraction(t_env *e, t_ray *ray, t_obj obj_tmp,
								size_t id);
void					reflexion(t_env *e, t_ray *ray, t_obj obj_tmp,
								size_t id);
cl_float3				rayon_refracte(cl_float3 normal, cl_float3 incident,
								float n1, float n2);
void					get_normal_object_sub(t_obj *obj, register t_ray ray,
								register float det);
void					get_normal_object(t_obj *obj, register t_ray ray,
								register float det);
