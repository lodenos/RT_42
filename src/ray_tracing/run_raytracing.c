								...
								float n1, float n2)
{
	...
	cosi = dot(normal, inv);
	cost = sqrt(1 - n * n * (1 - (cosi * cosi)));
	if (cosi > 0)
		t = add(vector_mult_x(normal, (cosi * n - cost)),
											...
											register float det)
{
	if (obj->type == CONE)
		obj->normal = reverse(normalize(sub(obj->pos_a, sub(obj->collision,
			vector_mult_x(vector_mult_x(obj->rotate, dot(ray.dir, obj->rotate)
			* det + dot(ray.pos, obj->rotate)),
			(1.0f + tanf(obj->angle / 2.0f) * tanf(obj->angle / 2.0f)))))));
