...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/11/01 18:52:59 by glodenos          #+#    #+#             */
/*   Updated: 2016/12/09 13:49:19 by glodenos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "lib_RT.h"

static void		get_info_position_sub(char **tmp)
{
	size_t	i;
	size_t	j;
	_Bool	k;
...
void			get_info_position(cl_float3 *pos, char *str)
{
	...
	get_info_position_sub(tmp);
	pos->x = (float)atof(tmp[0]);
	pos->y = (float)atof(tmp[1]);
	pos->z = (float)atof(tmp[2]);
	free_tab((void **)tmp);
}
